...
/**
 * @param adc_raw: The value of ADC, should notice this one. 
*/
esp_err_t err_adc;
int adc_timeout;
esp_err_t LM35_read_data(void){
     ...
     do{
     if(adc_timeout < 10){
     err_adc = adc_oneshot_read(adc1_handle, EXAMPLE_ADC1_CHAN0, &adc_raw);
     }
     if(err_adc == ESP_ERR_TIMEOUT) adc_timeout++;
     }while(err_adc != ESP_OK && adc_timeout < 10);
     if(adc_timeout == 10){
