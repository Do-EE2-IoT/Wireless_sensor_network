...
SemaphoreHandle_t Task_start_sem;
...
/**
 * @brief: This is struct will reg for gatt profile event handler
*/
    ...
    switch (event) {
        ...
        case ESP_GATTS_WRITE_EVT:
            if (!param->write.is_prep){
                ...
                if(param->write.value[0] == 0x61 && param->write.value[1] == 0x63 && param->write.value[2] == 0x6b ){
...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    if (event == ESP_GATTS_REG_EVT) {
        if (param->reg.status == ESP_GATT_OK) {
            ESP_LOGI(GATTS_TABLE_TAG, "register app successfully");
...
void Temperature_Measurement_Task(void * parameter){
    ...
    while(1){
        ...
        if(err_read_tem == ESP_ERR_TIMEOUT){
            do{
            ESP_LOGI(GATTS_TABLE_TAG, "Can't read tem, sending warning for gateway");
            // turn on led for mistake when read data sensor too much time
            if(send_indicate_timeout < 5){
            err_indicate = esp_ble_gatts_send_indicate(heart_rate_profile_tab[0].gatts_if,heart_rate_profile_tab[0].conn_id,
            heart_rate_handle_table[IDX_CHAR_VAL_A],2,(uint8_t *)"miss_tem",false);
            }
            ...
            else if(adc_raw >= 10 && adc_raw < 100){
                temp = malloc(2 * sizeof(char));
                *(temp + 1) = 0x00;
                *(temp + 2) = 0x00;
                *(temp + 3) = 0x00;
            }
            else if(adc_raw >= 100 && adc_raw < 1000){
            ...
            sprintf(temp,"%d",adc_raw);
            ESP_LOGI(GATTS_TABLE_TAG,"tem: %s and sizeof %d", temp, sizeof(temp));
        ...
        if(ble_send_time_out == 6){
            while(1){
            ESP_LOGE(GATTS_TABLE_TAG,"Bluetooth was broken, need to repair");
